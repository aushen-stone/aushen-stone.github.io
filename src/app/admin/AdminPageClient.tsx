...
function editorFromRow(row: CmsRow): EditorState {
  ...
  const pageKey = row.slug === "home" || row.slug === "services" || row.slug === "about" ? row.slug : null;
  ...
  const editableContent = pageKey && !("hero" in content)
    ? DEFAULT_LEGACY_PAGES[pageKey]
    : content;
...
function EditorPanel({ entity, editor, setEditor, busy, onSave, onUpload }: { entity: CmsEntityType; editor: EditorState; setEditor: React.Dispatch<React.SetStateAction<EditorState | null>>; busy: boolean; onSave: (event: React.FormEvent) => Promise<void>; onUpload: (file: File) => Promise<void> }) {
  ...
  if (PAGE_ENTITIES.has(entity)) {
    try { structuredContent = JSON.parse(editor.advancedJson) as Record<string, unknown>; } catch { structuredContent = null; }
  }
  const entityName = entity === "products" ? "product" : entity === "blog" ? "blog post" : entity === "projects" ? "project" : "page";
  return <div className="fixed inset-0 z-[1010] flex justify-end bg-black/20"><form onSubmit={onSave} className="h-full w-full max-w-2xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8"><div className="flex items-center justify-between"><h2 className="font-serif text-3xl">{editor.id ? "Edit" : "Add"} {entityName}</h2><button type="button" onClick={() => setEditor(null)} aria-label="Close editor"><X size={22} /></button></div><div className="mt-8 space-y-5"><Field label="Name / title"><input required value={editor.title} onChange={(event) => { update("title", event.target.value); if (!editor.id) update("slug", slugifyCmsValue(event.target.value)); }} className="admin-input" /></Field><Field label="Slug"><input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={editor.slug} onChange={(event) => update("slug", slugifyCmsValue(event.target.value))} className="admin-input" /></Field>{entity === "products" || entity === "projects" ? <Field label={entity === "products" ? "Material" : "Category"}><input required value={editor.secondaryLabel} onChange={(event) => update("secondaryLabel", event.target.value)} className="admin-input" /></Field> : !PAGE_ENTITIES.has(entity) ? <Field label="Categories (comma separated)"><input value={editor.categories} onChange={(event) => update("categories", event.target.value)} className="admin-input" /></Field> : null}<Field label="Status"><select value={editor.status} onChange={(event) => update("status", event.target.value)} className="admin-input"><option value="draft">Draft</option><option value="published">Published</option></select></Field>{!PAGE_ENTITIES.has(entity) ? <Field label={entity === "products" ? "Summary" : entity === "projects" ? "Description" : "Excerpt"}><textarea rows={4} value={editor.summary} onChange={(event) => update("summary", event.target.value)} className="admin-input py-3" /></Field> : null}{entity === "blog" ? <Field label="Article HTML"><textarea rows={12} required value={editor.bodyHtml} onChange={(event) => update("bodyHtml", event.target.value)} className="admin-input py-3 font-mono text-xs" /></Field> : null}<Field label="Image">{editor.imageUrl ? <img src={editor.imageUrl} alt="Current upload" className="mb-3 aspect-[16/9] w-full object-cover" /> : null}<label className="inline-flex cursor-pointer items-center gap-2 border border-[#D8D2C8] px-4 py-3 text-sm"><Upload size={16} /> Upload image<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void onUpload(file); }} /></label></Field>{structuredContent ? <section><h3 className="mb-4 font-serif text-2xl">Page sections</h3><StructuredContentEditor value={structuredContent as never} onChange={(value) => update("advancedJson", JSON.stringify(value, null, 2))} /></section> : null}<details className="border border-[#D8D2C8] p-4"><summary className="cursor-pointer text-sm font-medium">Advanced JSON</summary><p className="mt-2 text-xs leading-5 text-gray-500">The structured fields above and this JSON stay synchronized. Invalid JSON cannot be saved.</p><textarea rows={14} value={editor.advancedJson} onChange={(event) => update("advancedJson", event.target.value)} className="admin-input mt-3 py-3 font-mono text-xs" /></details></div><div className="sticky bottom-0 mt-8 flex gap-3 border-t border-[#E6E0D8] bg-white py-5"><button disabled={busy} className="flex h-12 flex-1 items-center justify-center bg-[#283020] text-xs uppercase tracking-[0.12em] text-white disabled:opacity-60">{busy ? <Loader2 className="animate-spin" size={18} /> : "Save changes"}</button><button type="button" onClick={() => setEditor(null)} className="h-12 border border-[#D8D2C8] px-6 text-sm">Cancel</button></div></form></div>;
}
