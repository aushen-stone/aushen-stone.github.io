...
const pages = SEO_LANDING_PAGES.filter((page) => page.kind === "application");
export function generateStaticParams() { return pages.map((page) => ({ slug: page.slug })); }
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = pages.find((item) => item.slug === slug);
  return page ? buildMetadata({ title: page.metaTitle, description: page.metaDescription, path: seoLandingPagePath(page) }) : buildMetadata({ title: "Page Not Found | Aushen Stone", path: `/products/application/${slug}/`, index: false });
}
export default async function ApplicationLandingPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const page = pages.find((item) => item.slug === slug);
  if (!page) notFound();
